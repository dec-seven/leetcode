/*
 * @Date: 2023-03-15 16:24:02
 * @LastEditTime: 2023-03-15 17:05:42
 * @Description: 
 */
/*
...
function maximalNetworkRank(n: number, roads: number[][]): number {
  const cityRoads =  Array.from({ length: n }, () => new Set<number>());
  ...
  for (let u = 0; u < n; u++) {
    for (let v = u + 1; v < n; v++) {
        ans = Math.max(ans, cityRoads[u].size + cityRoads[v].size - (cityRoads[u].has(v) ? 1 : 0));
    }
  }
  console.log(cityRoads,ans);
  return ans
};
// @lc code=end
